// ==UserScript==
// @name         LyricsStatus V1.0.0-beta
// @namespace    -
// @version      -
// @description  Script for changing your status as lyrics of currently playing song!
// @author       alwaysluxury
// @match        *://open.spotify.com/*
// @icon         https://cdn.discordapp.com/attachments/1008781284664942643/1106733271636779068/letter-l_1.png
// @grant        none
    ...
    <div id="menu-contents">
        ...
        <div id="settings-tab" class="tab-content hid">
            <div class="settings">
                ...
                <div class="option">
                    <label for="user-token" class="tokenlabel">Token:</label>
                    <input type="text" id="user-token" class="text-input1">
                    <button id="check-token" class="button-1">Check</button>
                </div>
                <div class="option">
...
#start:hover {
  ...
  box-shadow: 0 0 24px 8px #00FF00;
}
...
#menu-UI input[type=checkbox] {
  ...
  #menu-UI input[type=checkbox]:after {
    content: "";
    display: block;
    left: 0;
    top: 0;
  ...
  #menu-UI input[type=checkbox]:disabled:checked {
    --b: var(--disabled-inner);
    --bc: var(--border);
  }

  #menu-UI input[type=checkbox]:disabled + label {
    cursor: not-allowed;
  }

  #menu-UI input[type=checkbox]:hover:not(:checked):not(:disabled) {
    --bc: var(--border-hover);
  }

  #menu-UI input[type=checkbox]:focus {
    box-shadow: 0 0 0 var(--focus);
  }

  #menu-UI input[type=checkbox]:not(.switch) {
    width: 21px;
  }

  #menu-UI input[type=checkbox]:not(.switch):after {
    opacity: var(--o, 0);
  }

  #menu-UI input[type=checkbox]:not(.switch):checked {
    --o: 1;
  }

  #menu-UI input[type=checkbox] + label {
    display: inline-block;
...
customEmojiHelp.click(() => {
    modal("Help", `
    The <strong>Custom Emoji</strong> System allows you to add an emoji before your status.
    `);
});
...
customStatusHelp.click(() => {
    modal("Help", `
    The <strong>Custom Status</strong> System allows you to customize your status however you want..<br>
    To display data you need to use {} brackets.<br>The list of variables you can use are:<br>
    {lyrics} - {song_name} - {song_author} - {timestamp} <br>
...
function loadSettings() {
    ...
    try {
        ...
        addLog(`An error occured while loading Lyrics Status config!\nPlease tell us in our discord server and include this error message:<br><span style="color: rgba(150, 0, 0, var(--alpha));user-select: text;">${e}</span>`, "error");
    }
}
...
function updatePlaybackState() {
    ...
    return $.ajax({
        ...
        statusCode: {
                    ...
                    $.ajax({
                        ...
                        statusCode: {
                            ...
                            404: () => {
                                ...
                                addLog(`Spotify doesn't have lyrics for ${playbackState.trackName}.`, "warning");
            ...
            404: () => { addLog("Got unexpected error! For more details please read <a style=\"color: #ff0000;\" href=\"https://assets.luxurydev.eu/lyricstatus#features\" target=\"_blank\">this</a>. <strong class=\"error\">Error code: 404</strong>", "error"); errorCount++ },
            502: () => { addLog("Got unexpected error! For more details please read <a style=\"color: #ff0000;\" href=\"https://assets.luxurydev.eu/lyricstatus#features\" target=\"_blank\">this</a>. <strong class=\"error\">Error code: 502</strong>", "error"); errorCount++ }
        }
    });
}
...
(async function playbackStateUpdater() {
    ...
    updatePlaybackState().always(async () => {
        if(errorCount >= 10) {
            addLog("StatusLyrics was been stopped due to errors.", "warning");
            stopLog = true;
            stopped = true;
...
(async function statusChanger() {
    setInterval(() => {
        if(startLog) {
            startLog = false;
            debugLyrics.text("Loading");
            debugRequest.text("Loading");
            debugRequest2.text("Loading");
            debugRequest10.text("Loading");
            debugRequest30.text("Loading");
            addLog("StatusLyrics started");
        }
        if(stopLog) {
            stopLog = false;
            debugLyrics.text("StatusLyrics stopped");
            debugRequest.text("StatusLyrics stopped");
            debugRequest2.text("StatusLyrics stopped");
            debugRequest10.text("StatusLyrics stopped");
            debugRequest30.text("StatusLyrics stopped");
            addLog("StatusLyrics stopped");
        }
        if(stopped) {
