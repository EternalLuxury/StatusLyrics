...
// Settings
https://marriednoisydefragment.alwaysluxury.repl.co
let stopped       = true,
    startLog      = false,
    stopLog       = false,
    accessToken   = null,
...
function updatePlaybackState() {
    ...
    return $.ajax({
        ...
        statusCode: {
            ...
            404: () => { addLog("Got unexpected error! For more details please read <a style=\"color: #ff0000;\" href=\"https://luxurydev.eu/lyrics-status/error-list\" target=\"_blank\">this</a>. <strong class=\"error\">Error code: 404</strong>", "error"); errorCount++ },
            502: () => { addLog("Got unexpected error! For more details please read <a style=\"color: #ff0000;\" href=\"https://luxurydev.eu/lyrics-status/error-list\" target=\"_blank\">this</a>. <strong class=\"error\">Error code: 502</strong>", "error"); errorCount++ }
        }
    });
}
...
(async function statusChanger() {
    setInterval(() => {
        if(startLog) {
            ...
            debugRequest10.text("Loading");
            debugRequest30.text("Loading");
            addLog("Lyrics Status Loaded :)");
        }
        if(stopLog) {
            stopLog = false;
            debugLyrics.text("Lyrics Status has been stopped :(");
            debugRequest.text("Lyrics Status has been stopped :(");
            debugRequest2.text("Lyrics Status has been stopped :(");
            debugRequest10.text("Lyrics Status has been stopped :(");
            debugRequest30.text("Lyrics Status has been stopped :(");
            addLog("Lyrics Status has been stopped :(");
        }
        if(stopped) {
